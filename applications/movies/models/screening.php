...
class movies_screening_model extends web_model
{
	...
	public function remove_screening($screening_id)
	{
		...
		$conds[] = 'NOT EXISTS (SELECT * FROM movie_screenings WHERE screening_id = ' . (int) $screening_id . ')';
	...
	{
		$conds = array();
		$conds[] = ($only_upcoming ? 'NOT EXISTS (SELECT * FROM movie_screenings WHERE screening_start < ' . time() . ')' : false);
